...
fn get_key() -> String {
    std::env::var("FEVER_ZAI_KEY")
        .expect("FEVER_ZAI_KEY env var must be set for integration test")
}
...
async fn test_openrouter_fetch_models() {
    ...
    adapter.fetch_models().await.expect("fetch_models should succeed");

    let models = adapter.list_models();
    assert!(!models.is_empty(), "Should have fetched at least some models");
...
async fn test_openrouter_chat() {
    ...
    adapter.validate_config().await.expect("config should be valid");
    ...
    match result {
        ...
        Err(e) => {
            ...
            if err_str.contains("Auth") || err_str.contains("401") || err_str.contains("403") {
                ...
                println!("   Model listing (GET /models) works. Chat (POST /chat/completions) needs a key with write access.");
            } else {
