...
impl TelegramApi {
    ...
    pub async fn send_message(&self, chat_id: &str, text: &str) -> Result<(), TelegramError> {
        ...
        if resp.status().is_success() {
            ...
            Err(TelegramError::Api { code: status, message: body })
        }
    }
    ...
    pub async fn get_updates(&self, offset: Option<i64>) -> Result<Vec<Value>, TelegramError> {
        ...
        let resp = req.send().await.map_err(|e| TelegramError::Network(e.to_string()))?;
        if resp.status().is_success() {
            let json: Value = resp.json().await.map_err(|e| TelegramError::Api { code: -1, message: e.to_string() })?;
            let updates = json.get("result").and_then(|v| v.as_array()).cloned().unwrap_or_else(|| vec![]);
            Ok(updates)
        } else {
            let status = resp.status().as_u16() as i32;
            let body = resp.text().await.unwrap_or_else(|_| "unknown error".to_string());
